...
17. **Bash 工具里 `cmd /c xxx.bat` 会被 MSYS 路径转换吃掉**：`/c` 被转成 `C:/`，cmd 只打印版本横幅就退出、脚本静默不执行（`restart-backend.bat` 看着像跑过了，其实服务没起）。正确写法 `MSYS2_ARG_CONV_EXCL='*' cmd /c xxx.bat`；是否真执行看 bat 自己的回显（如 `[restart-backend] spawned and exited`），别只看退出码。
18. **门户（/api/sit）路径前缀反代的两类相对引用不可混用**（2026-09-23 排查）：门户 loader 是 `bootPATH + jss 条目` **字符串拼接**（`awsui.js` 的 `loadjs`），这类模块路径（如 app 目录的 `../../../commons/js/util/Base64.js`）必须**保持相对**——前缀恰好两段，归一化后正好落回 `/api/sit/commons/…`；改成绝对地址会被拼成 `/api/sit/apps/https%3A//…` 而 403。而 `document.write` 的 `<script src>`、`url()`、`img src` 是**文档相对**（真站靠浏览器在站点根钳位解析），必须改成前缀绝对，否则掉前缀。**验证手法**：`ssh -L 10099:127.0.0.1:10003 root@…` 转发容器 SOCKS 后本地起后端 `SIT_PORTAL_SOCKS=127.0.0.1:10099`，用 `?t=<JWT>` 走一遍 `/api/sit/<资源>` 取真实字节比对；门户原始字节同样可 `curl --socks5-hostname 127.0.0.1:10003` 在服务器上取。
